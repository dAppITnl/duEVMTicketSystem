...
const SeatChart = ({ occasion, duEVMTicketSystem, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await duEVMTicketSystem.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await duEVMTicketSystem.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
